...
from resources.data import RAID, REPLYRAID, FIXXSPAM, BRTHSPAM
from FIXXSPAM import BOT0, BOT1, BOT2, BOT3, BOT4, BOT5, BOT6, BOT7, BOT8, BOT9, SUDOERS
...
async def spam(e):
    ...
    if e.sender_id in SUDOERS:
        ...
        FIXX = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        bitxh = await e.get_reply_message()
        if len(FIXX) == 2:
            user = str(FIXX[1])
            a = await e.client.get_entity(user)
            g = a.id
            if int(g) in FIXXSPAM:
   ...
   if event.sender_id in SUDOERS:
         ...
         if len(FIXX) == 3:
             ...
             if int(e) in FIXXSPAM:
                 ...
                 counter = int(FIXX[1])
                 sleeptimet = sleeptimem = float(FIXX[0])
                 for _ in range(counter):
                      reply = random.choice(RAID)
                      caption = f"{username} {reply}"
                      async with event.client.action(event.chat_id, "typing"):

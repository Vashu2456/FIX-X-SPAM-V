...
async def spam(e):
    usage = "𝗠𝗼𝗱𝘂𝗹𝗲 𝗡𝗮𝗺𝗲 = 𝙁𝙄𝙓𝙓𝗦𝗽𝗮𝗺\n\nCommand:\n\n.delayspam <sleep time> <count> <message to spam>\n\n.delayspam <sleep time> <count> <reply to a message>\n\nCount and Sleeptime must be a integer."     
    if e.sender_id in SUDOERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage)
        smex = await e.get_reply_message()
        FIXX = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        FIXXsexy = FIXX[1:]
        if len(FIXXsexy) == 2:
            message = str(FIXXsexy[1])
            counter = int(FIXXsexy[0])
            sleeptime = float(FIXX[0])
            for _ in range(counter):
                async with e.client.action(e.chat_id, "typing"):
                    if e.reply_to_msg_id:
                        await smex.reply(message)
    ...
    if e.sender_id in SUDOERS:
        ...
        if len(FIXX) == 1:
            ...
            if int(e.chat_id) in GROUP:
                text = f"ꜱᴘᴀᴍᴍɪɴɢ ɪɴ ᴠᴀꜱʜᴜ x ꜰɪx ᴄʜᴀᴛꜱ ɪꜱ ɪʟʟɪɢᴀʟ"
                await e.reply(text)
            else:
